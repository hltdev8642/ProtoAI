...
function opts()
{
   ...
   opts.config = function()
   {
      ...
      //Scripts and Plugins
      opts.LoadPlugins(app.ReadFile("plugins.txt"), false);
      uix = app.CreateUIExtras();
...
function onFileChoose(file)
{
   ...
   if ((file.indexOf(".jpg") > -1) | (file.indexOf(".png") > -1) | (file.toLowerCase().indexOf(".jpeg") > -1) | (file.indexOf(".svg") > -1) | (file.indexOf(".psd") > -1) | (file.indexOf(".bmp") > -1) | (file.indexOf(".gif") > -1) | (file.indexOf(".ico") > -1) | (file.indexOf(".cr2") > -1) | (file.indexOf(".raw") > -1))
   {
      ...
      app.MakeFolder( "/sdcard/temp" )
   //   filenametmp = (file.split("/")
   const filename3 = file.match(/[^/]+$/)[0];
   ...
   if (file.indexOf(".py") > -1)
   {
      ...
      {
      outtxt = app.SysExec( "cat " + file + ";exit;", "su,log" )
      edtTxt.SetText(outtxt)
      }
      app.SaveText("currentFile", file, "currentFile.txt");
...
function extraKeyBar()
{
   ...
   {
      this.editor = editor
      var keys = this.keys || app.ReadFile( "keys.txt" );
...
// Link:[https://groups.google.com/d/msg/droidscriptpremium/J95-ER0xZ7I/7ZjoElG3AAAJ]
//------------------------------------------------------------------------
...
FolderLister.prototype.MakeList = function(options, textSize)
{
   ...
   list = list.split("\n").join('\0')
   //      this.list = list
   this.list = app.CreateList(list, 1, 0.45, "", '\0');
